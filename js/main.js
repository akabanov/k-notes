...
const epubMakerBtn = document.querySelector('#epubMakerBtn');
...
initSqlJs({
  locateFile: filename => `js/vendor/${filename}`
}).then(function (SQL) {

  const fileElement = document.querySelector("#koboDatabase");

  fileElement.onchange = () => {
    const file = fileElement.files[0];
    const reader = new FileReader();
    reader.onload = function () {
      const dbBlob = new Uint8Array(reader.result);
      const db = new SQL.Database(dbBlob);


      const stmt = db.prepare("select distinct (BookTitle) from content");
      while (stmt.step()) { //
